...
export default function EditarDespachoPage({ params }: PageProps) {
  const resolvedParams = use(params)
  const numeroId = decodeURIComponent(resolvedParams.id)
  ...
  const [formData, setFormData] = useState({
    ...
    descripcion: '',
    cuit_importador: '',
    aduana_registro: 'IC05',
    valor_fob: '',
    flete: '',
    seguro: '',
    incoterm: 'CPT',
    pais_destino: '203',
    tipo_doc: 'FACTURACOMERCIAL',
    ref_doc: '',
    fecha_emision_fact: '',
    domicilio_establec: '',
    localidad_prov: '',
  ...
  useEffect(() => {
    const fetchDespacho = async () => {
      ...
      if (error) {
        alert(`Error al cargar el despacho: ${error.message}`)
        router.push('/despachos')
      } else if (data) {
        setFormData({
          ej: data.ej || '26',
          tipo: data.tipo || 'GMCI',
          numero: data.numero || numeroId,
          cliente: data.cliente || '',
          ref_cliente: data.ref_cliente || '',
          destinacion: data.destinacion || '',
          fecha: data.fecha || '',
          descripcion: data.descripcion || '',
          cuit_importador: data.cuit_importador || '',
          aduana_registro: data.aduana_registro || 'IC05',
          valor_fob: data.valor_fob || '',
          flete: data.flete || '',
          seguro: data.seguro || '',
          incoterm: data.incoterm || 'CPT',
          pais_destino: data.pais_destino || '203',
          tipo_doc: data.tipo_doc || 'FACTURACOMERCIAL',
          ref_doc: data.ref_doc || '',
          fecha_emision_fact: data.fecha_emision_fact || '',
          domicilio_establec: data.domicilio_establec || '',
          localidad_prov: data.localidad_prov || '',
          fecha_inic_activ: data.fecha_inic_activ || '',
          unidades: data.unidades || '',
          precio_unitario: data.precio_unitario || '',
          peso_neto: data.peso_neto || '',
          ncm: data.ncm || '',
          descripcion_sbt: data.descripcion_sbt || '',
        })
      }
      setLoading(false)
    }
...
`
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    handleDescargarTxt()
    setGuardando(false)
    alert('¡Despacho actualizado y archivo TXT para MARIA generado con éxito!')
    router.push('/despachos')
  }

  if (loading) {
    return (
      <div className="w-full max-w-5xl mx-auto p-12 text-center text-gray-500">
        Cargando datos del despacho para editar...
      </div>
    )
  }
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-6 pb-12">
      <header className="flex justify-between items-center border-b border-gray-200 dark:border-slate-800 pb-4">
        <div>
          <h1 className="text-xl font-black text-gray-900 dark:text-white">Editar Despacho: {numeroId}</h1>
          <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">
            Actualice los datos de la operación aduanera y regenere el TXT oficial.
          </p>
        </div>
        <Link
          href="/despachos"
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input
                ...
                disabled
                className="w-full mt-1 p-2.5 bg-gray-100 dark:bg-slate-800/40 border border-gray-200 dark:border-slate-700 rounded-xl text-xs font-bold text-gray-500 dark:text-slate-400 cursor-not-allowed"
              />
            </div>
          </div>
        ...
        <div className="flex justify-end gap-3 pt-2">
          <button
            type="submit"
            disabled={guardando}
            className="bg-sky-600 hover:bg-sky-700 text-white px-6 py-3 rounded-xl text-xs font-bold transition shadow-xs flex items-center gap-2 cursor-pointer disabled:opacity-50"
          >
            {guardando ? 'Guardando cambios...' : '💾 Actualizar Despacho y Descargar TXT'}
          </button>
        </div>
      </form>
    </div>
